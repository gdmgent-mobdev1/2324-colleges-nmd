...
const getImageButtons = (sprites: SpriteSet, activeKey: string) => {
  let buttons = "";
...
const setDetailLayout = (pokemon: Pokemon, spriteKey: string) => {
  if (!$detail) {
    return;
  }
  ...
  $detail.innerHTML = `
    ...
    pokemon.name
      ...
      ${getImageButtons(pokemon.sprites.other, spriteKey)}
...
const render = (id?: string | null) => {
  ...
  fetchPokemonById(id)
    .then((data) => {
      if (!isVoid(data)) {
        setDetailLayout(data, spriteKey);
        addEventListeners();
      }
    })
    .catch((error) => {
